...
export function DSCPSProvider({ children }: { children: ReactNode }) {
  ...
  const [users, setUsers] = useState<UserProfile[]>(() => {
    ...
    return saved ? JSON.parse(saved) : INITIAL_USERS.map(u => ({
      ...u,
      points: u.uid === 'admin-1' ? 450 : u.uid === 'user-2' ? 350 : 120
    }));
  });
  ...
  const completeWorkload = (workloadId: string) => {
    ...
    if (targetWork.status !== 'completed') {
      setUsers(prev => prev.map(u => u.uid === targetWork.assignedToId ? {
        ...u,
        points: (u.points || 0) + targetWork.pts
      } : u));
    }
  };
  ...
  const abandonWorkload = (workloadId: string, subtractPoints: boolean) => {
    ...
    if (subtractPoints) {
      setUsers(prev => prev.map(u => u.uid === targetWork.assignedToId ? {
        ...u,
        points: Math.max(0, (u.points || 0) - targetWork.pts)
      } : u));
    }
  };
