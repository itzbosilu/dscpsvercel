...
export default function Corners({ initialActiveSection, onNavigateToProfile }: CornersProps) {
  ...
  // Editors Corner Sub tabs state
  const [editorsTab, setEditorsTab] = useState<'workloads' | 'calendar' | 'essays'>('workloads');
  ...
  const handlePublishArticle = (e: FormEvent) => {
    e.preventDefault();
    if (!currentUser || !artTitle || !artContent) return;

    addArticleToCorner(artTitle, artContent, currentUser.fullName, currentUser.uid);
    setArtTitle('');
    setArtContent('');
              ...
              {showPhotoForm && (
                <div id="photo-submit-form-panel" className="bg-neutral-950 border border-neutral-900 rounded-2xl p-5 md:p-6 mb-8 max-w-lg mx-auto animate-fade-in text-left">
                  ...
                  <form onSubmit={handleUploadPhoto} className="space-y-4">
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5">Exhibition Title</label>
                      <input
                        type="text"
                        required
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5">Capture Description / Exif details</label>
                      <textarea
                        rows={3}
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-neutral-400 uppercase tracking-widest mb-1.5">Frame Unsplash/Direct Image URL</label>
                      <input
                        type="url"
                        required
                      ...
                      />
                      <span className="text-[10px] text-neutral-500 font-mono mt-1 block">💡 You can copy any high-res image address from Unsplash or upload assets via cloud channels.</span>
                    </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center justify-between border-b border-neutral-900 pb-3 mb-6 gap-4">
                <div className="flex space-x-1.5 bg-neutral-950/60 p-1 rounded-xl border border-neutral-900/80">
                  <button
                    onClick={() => setEditorsTab('workloads')}
                    className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
                      editorsTab === 'workloads'
                        ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold'
                        : 'text-neutral-400 hover:text-white'
                    }`}
                  >
                    <ClipboardList size={13} />
                    <span>Workloads Hub</span>
                  </button>

                  <button
                    onClick={() => setEditorsTab('calendar')}
                    className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
                      editorsTab === 'calendar'
                        ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold'
                        : 'text-neutral-400 hover:text-white'
                  ...
                  >
                    <Calendar size={13} />
                    <span>Lankan Calendar</span>
                  </button>

                  <button
                    onClick={() => setEditorsTab('essays')}
                    className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer ${
                      editorsTab === 'essays'
                        ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 font-bold'
                        : 'text-neutral-400 hover:text-white'
              ...
              {/* TAB 2: INTERACTIVE SRI LANKAN CALENDAR */}
              {editorsTab === 'calendar' && (
              ...
              {/* TAB 3: ORIGINAL COLUMN ARTICLES ESSAYS */}
              {editorsTab === 'essays' && (
