          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end space-y-4 sm:space-y-0 sm:space-x-5 text-center sm:text-left">
            ...
            <div className="mb-2">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                ...
                <span className="text-[10px] font-bold font-mono bg-amber-500/15 text-amber-400 border border-amber-500/35 px-2.5 py-0.5 rounded-full flex items-center gap-1 shadow-inner">
          ...
          <div className="bg-neutral-950 border border-neutral-900 rounded-3xl p-6">
            ...
            {isEditing ? (
              <form onSubmit={handleSaveProfile} className="space-y-4 font-sans text-left">
                <div>
                  <label className="block text-[10px] uppercase font-mono tracking-wider text-neutral-400 mb-1">Biography Description</label>
                  <textarea
                    rows={4}
                    value={editedBio}
                    onChange={e => setEditedBio(e.target.value)}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-lg p-2 text-xs text-white focus:outline-none focus:border-amber-500 font-sans"
                    placeholder="Describe your student camera gears, Lightroom goals, or portfolio ambitions..."
                  />
                </div>

                <div>
                  <label className="block text-[10px] uppercase font-mono tracking-wider text-neutral-400 mb-1">Avatar Image URL</label>
                  <input
                    type="url"
                    value={editedAvatar}
                    onChange={e => setEditedAvatar(e.target.value)}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-lg p-2 text-xs text-white focus:outline-none focus:border-amber-500 font-mono"
                    placeholder="Avatar address link"
                  />
                </div>

                <div>
                  <label className="block text-[10px] uppercase font-mono tracking-wider text-neutral-400 mb-1">Background Banner URL</label>
                  <input
                    type="url"
                    value={editedBanner}
                    onChange={e => setEditedBanner(e.target.value)}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-lg p-2 text-xs text-white focus:outline-none focus:border-amber-500 font-mono"
                    placeholder="Banner address link"
                  />
                </div>

                <div>
                  <label className="block text-[10px] uppercase font-mono tracking-wider text-neutral-450 text-neutral-400 mb-1">Instagram Link</label>
                  <input
                    type="url"
                ...
                <div>
                  <label className="block text-[10px] uppercase font-mono tracking-wider text-neutral-450 text-neutral-400 mb-1">Behance Link</label>
                  <input
                    type="url"
                    value={editedBehance}
              ...
              <div>
                ...
                <div className="flex flex-col gap-2 mt-6 pt-5 border-t border-neutral-900">
                  <p className="text-[10px] uppercase font-mono text-neutral-550 text-neutral-500 tracking-widest font-bold">Connect & Portfolio</p>
                  ...
                  {targetUser.instagramUrl ? (
                    ...
                    <div className="text-[11px] text-neutral-600 font-mono">No Instagram handle configured.</div>
                  )}
                  ...
                  {targetUser.behanceUrl ? (
                    ...
                    <div className="text-[11px] text-neutral-600 font-mono mt-1">No Portfolio link configured.</div>
                  )}
                ...
                {filteredExposures.map(photo => (
                  ...
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-neutral-950 via-neutral-950/40 p-4">
                      ...
                      <p className="text-[9px] text-neutral-500 font-mono mt-0.5 text-left">Approved framing logged on {photo.uploadedAt}</p>
                    </div>
                  </div>
                ))}
              </div>
